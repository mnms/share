...
- v1.0.0
<pre><code>Tango 운영에 적용된 버전을 기능 변경없이 코드 정리한 버전

반영 사항:

- tsr2-etl에서 server status check(PING) 및 connectTempate(JobId) 시도 시 retry 기능 추가

- 'flashbase -version'으로 버전 확인 커맨드 추가 </code></pre>


- v1.1.1
<pre><code>Bharti BMT 검증 버전

반영 사항:

- rowStore 적용

- 'IN' query 방식 변경 </code></pre>


- v1.1.9
<pre><code>기능 안정화 및 성능 개선 버전

반영 사항:

- Build/Runtime configuration 정리

- 이중화 관련 버그 수정

- GOSSIP MESSAGE 성능 개선
</code></pre>


- v1.1.13 (recommanded)
<pre><code>loader refactoring
...
```  console
v1.1.13 이후 버전은 기본 라이센스 기간이 7일로 제한되며, 추가 배포되는 라이센스 키 파일로 기간을 연장해주어야 함
버전명도 'flashbase.{dev/trial/official}.{branch name}.{마지막 commit 앞 6자리}.bin'으로 naming rule을 변경함
